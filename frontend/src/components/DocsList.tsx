...
export default function DocsList({titleName, contentName, label, open, onShow}: props) {
	...
	return (
		<section className="flex flex-col relative w-300 h-20 gap-1 border-b border-b-gray-300 mt-15">
			<p className="text-lg text-gray-800 font-semibold">{titleName}</p>
			<p className="text-md text-gray-600">{contentName}</p>
		
			<button onClick={onShow} className={` ${open ? 'active' : ''} flex items-center justify-center gap-1 w-37 h-7 mt-4 border border-gray-300 rounded-full absolute right-2 hover:bg-slate-100/80 cursor-pointer`}>{label}
				<span>
