...
export default function CreatePostForm({title, content, open, onClick, onShow}: formTypes) {
	return (
		<div className="mt-5 h-120 w-100 ml-20 p-5 shadow-[0_0_20px_rgba(168,85,247,0.3)] rounded-xl" style={{ background: 'radial-gradient(circle at 0% 0%, #9B7BD5 0%, #B8A3E6 30%, transparent 60%), radial-gradient(circle at 100% 100%, #5B8FE8 0%, #8FB9F0 30%, transparent 60%), radial-gradient(ellipse at 100% 0%, #A88FD5 0%, transparent 55%), radial-gradient(ellipse at 0% 100%, #7AACF7 0%, transparent 55%), #D5C9F0' }}>
			<div onClick={onShow} className={` ${open ? 'active' : ''} flex flex-col h-110 w-90 bg-[#E0D3ED] shadow-[0_0_20px_rgba(168,85,247,0.3)] rounded-md`}>
				...
				<input className="pl-2 h-10 w-80 ml-5 bg-[#F3F0FA] rounded-md focus:outline-none" type="text" placeholder="Title">{title}</input>
				<textarea className="mt-8 mx-5 pl-2 h-45 bg-[#F3F0FA] rounded-md focus:outline-none" placeholder="Content">{content}</textarea>
				<button onClick={onClick} className="flex items-center h-12 w-80 mt-10 mx-5 bg-[linear-gradient(to_right,#A855F7,#9333EA,#7C3AED)] rounded-md text-white text-md font-semibold cursor-pointer">
